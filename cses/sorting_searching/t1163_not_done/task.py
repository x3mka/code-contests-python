...
def main():
    ...
    ranges = TreapSet([(0, x)])
    lengths = TreapMultiSet([x])

    for i in range(n):
        seg = ranges.floor((a[i], 0))
        ranges.remove(seg)
        ranges.add((seg[0], a[i]))
        ranges.add((a[i], seg[1]))
        lengths.remove(seg[1]-seg[0])
        lengths.add(a[i]-seg[0])
        lengths.add(seg[1]-a[i])
        res.append(lengths.max())
