...
class SegmentTree:
    ...
    def _fix_node(self, idx):
        self.data[idx] = self._combine_fn(self.data[self._left(idx)], self.data[self._right(idx)])
...
class MinCountSegmentTree(SegmentTree):
    ...
    def __init__(self, data):
        SegmentTree.__init__(self, data, combine_fn=combine, default_leaf_fn=lambda x: (x, 1), default_node_val=(sys.maxsize, 0))


def main():
    n, m = ria()
    a = ria()
    st = MinCountSegmentTree(a)
    ...
    for _ in range(m):
        ...
        if op == 1:
            st[x] = y
        if op == 2:
            out.write(ia_to_s(list(st.query(x, y-1))) + "\n")
    print(out.getvalue())
