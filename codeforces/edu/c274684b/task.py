...
# x and y are tuples of 4 numbers (,,,) representing matrix 2x2
def mul(r, x, y):
    return (x[0]*y[0]+x[1]*y[2]) % r, (x[0]*y[1]+x[1]*y[3]) % r, (x[2]*y[0]+x[3]*y[2]) % r, (x[2]*y[1]+x[3]*y[3]) % r
...
def main():
    ...
    a = []
    for _ in range(n):
        a.append(tuple(ria() + ria()))
        rs()

    combine = partial(mul, r)
    st = SegmentTree(a, combine_fn=combine, default_node_val=(1, 0, 0, 1))

    for _ in range(m):
        xx = ria()
        res = st.query(xx[0]-1, xx[1])
        wia([res[0], res[1]])
        wia([res[2], res[3]])
        ws('')
