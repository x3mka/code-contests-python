...
class SegmentTree:
    ...
    def _lower_bound(self, x, left, right, idx, lx, rx):
        """Finds min index j in [left, right] such that a[j] >= x; -1 - if not found; j is 0-based"""
        # outside
        if right < lx or left > rx:
            return -1

        if lx == rx:
            return lx if self.data[idx] >= x else -1

        left_max = self.data[2 * idx]
        mid = (lx + rx) // 2
        if left_max >= x:  # needed index can be in left segment (but can be missing too)
            res = self._lower_bound(x, left, right, 2 * idx, lx, mid)
            if res == -1:
                return self._lower_bound(x, left, right, 2 * idx + 1, mid+1, rx)
            else:
                return res
        else:
            return self._lower_bound(x, left, right, 2 * idx + 1, mid+1, rx)

    def lower_bound(self, x, left):
        """Finds min index j >= left such that a[j] >= x; -1 - if not found; j is 0-based"""
        return self._lower_bound(x, left, self._size-1, 1, 0, self._size - 1)


class MaxSegmentTree(SegmentTree):

    def __init__(self, data, default=-sys.maxsize-1):
        SegmentTree.__init__(self, data, combine_fn=lambda x, y: max(x, y), default_node_val=default)
