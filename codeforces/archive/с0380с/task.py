...
def main():
    s = rs()
    a = [1 if si == '(' else -1 for si in s]
    n = len(a)
    st = SegmentTree(a, combine_fn=combine, default_leaf_fn=lambda x: (0, 1 if x == 1 else 0, 1 if x == -1 else 0), default_node_val=(0, 0, 0))

    m = ri()
    for _ in range(m):
        l, r = ria()
        wi(2 * st.query(l-1, r)[0])
